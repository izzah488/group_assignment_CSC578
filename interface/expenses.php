...
try {
    $sql = "SELECT name, lastname, userimage FROM users WHERE id = :userId";
    ...
    if ($userProfile) {
        ...
        if (!empty($userProfile['userimage'])) {
...
<body class="bg-gray-100 flex">
    ...
    <aside class="sidebar">
        <div class="flex flex-col justify-start h-full">
            <div class="flex items-center mb-8 user-panel">
                <div class="image">
                    <img src="staff_images/<?php echo htmlentities($userImage); ?>" alt="User profile picture">
                </div>
                <div class="info">
                    <p class="text-sm font-medium text-gray-700">Hi, <?php echo htmlentities($userName); ?>!</p>
                    <p class="text-xs text-gray-500">Money Mate User</p>
                </div>
            </div>
            <button onclick="window.location.href='indexS.php'" class="menu-btn w-full mb-4">
                ☰ Dashboard
            </button>
