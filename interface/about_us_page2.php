...
<body>
  ...
  <button id="scrollBtn" class="scroll-top-btn" aria-label="Scroll">
    <svg id="scrollBtnIcon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-7 h-7">
      <path id="scrollBtnPath" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
    </svg>
  </button>
  <script>
    // Scroll button logic: show down if near top, up if scrolled down
    const scrollBtn = document.getElementById('scrollBtn');
    const scrollBtnIcon = document.getElementById('scrollBtnIcon');
    const scrollBtnPath = document.getElementById('scrollBtnPath');
    const contactSection = document.getElementById('contact-section');
    function updateScrollBtn() {
      if (window.scrollY < 100) {
        // Show scroll down
        scrollBtn.onclick = () => contactSection.scrollIntoView({behavior: 'smooth'});
        scrollBtn.setAttribute('aria-label', 'Scroll to contact section');
        scrollBtnPath.setAttribute('d', 'M5 9l7 7 7-7'); // Down arrow
      } else {
        // Show scroll up
        scrollBtn.onclick = () => window.scrollTo({top:0,behavior:'smooth'});
        scrollBtn.setAttribute('aria-label', 'Scroll to top');
        scrollBtnPath.setAttribute('d', 'M5 15l7-7 7 7'); // Up arrow
      }
    }
